...
class CookieException(CustomException):
    ''' 例:
        ...
        raise CookieException(method_name="get_max_age()", message=f"有効なセッションがありません {str(e)}")    
    '''
    def __init__(
        self,
        method_name: str,
        ...
        logger.error(f" - {status.HTTP_500_INTERNAL_SERVER_ERROR} - Cookie情報が取得できませんでした - {method_name} - {detail}")
